...
const Home = () => {
  ...
  const filtered = (foodItem || [])
    .filter(item => filter === "all" || (filter === "veg" ? item.veg === true : item.veg === false))
  ...
  return (
    <div className="min-h-screen bg-[#040f1a]">
      ...
      <nav className={`sticky top-0 z-50 transition-all duration-300 ${
        scrolled ? "bg-[#040f1a]/85 backdrop-blur-xl border-b border-white/[0.06] shadow-[0_4px_30px_rgba(0,0,0,0.5)]"
        ...
        <div className="max-w-[1240px] mx-auto flex items-center gap-5 h-16 px-6">
          ...
          <div className="flex items-center gap-2.5 flex-shrink-0">
            ...
            <img className="h-7 w-auto" src="logo-no-background.png" alt="Logo" />
          </div>
          ...
          <div className="flex-1 max-w-[460px] flex items-center gap-2.5 bg-white/[0.05] border border-white/[0.09] rounded-full px-4 h-10 focus-within:border-[#58ffcd]/40 focus-within:bg-white/[0.07] transition-all duration-200">
            <span className="text-[#81FFD9]/40 flex-shrink-0 flex"><SearchIcon /></span>
            <input
            ...
            />
            {search && (
              <button onClick={() => setSearch("")} className="text-[#81FFD9]/30 hover:text-[#81FFD9]/70 transition-colors flex-shrink-0">
          ...
          {isLoggedIn ? (
            ...
            <div className="flex items-center gap-2.5">
              <Link to="/login" className="border border-white/15 text-white/65 hover:text-white hover:border-white/30 font-semibold px-5 py-2 rounded-full transition-all duration-200 text-sm">
                Log In
              </Link>
              <Link to="/signup" className="bg-[#58ffcd] text-[#040f1a] hover:bg-[#7bffd9] font-bold px-5 py-2 rounded-full transition-all duration-200 text-sm shadow-[0_0_18px_rgba(88,255,205,0.35)]">
                Sign Up
              </Link>
            </div>
          )}
      ...
      <div className="relative bg-gradient-to-r from-[#051726] via-[#082540] to-[#051726] border-b border-[#58ffcd]/[0.07] overflow-hidden">
        ...
        <div className="max-w-[1240px] mx-auto px-6 py-10">
          ...
          <p className="text-[#81FFD9]/50 text-[15px] max-w-md">
            Fresh, hot meals delivered to your door. Browse our menu and order in minutes.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-[1240px] mx-auto px-6 pt-7 pb-16">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-[#e0fff8] text-xl font-extrabold tracking-tight">Our Menu</h2>
            <p className="text-[#81FFD9]/40 text-[13px] mt-0.5">{filtered.length} dishes available</p>
          </div>
        ...
        {filtered.length > 0 ? (
          <div className="flex flex-wrap gap-5">
            {filtered.map(item => <Card key={item._id || item.id} food={item} price={item.price} />)}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-24 text-center gap-3">
            <span className="text-5xl">🍽️</span>
            <h3 className="text-[#e0fff8] text-lg font-bold">Nothing found</h3>
            <p className="text-[#81FFD9]/40 text-sm">Try a different search or filter</p>
            <button
